...
export default function SharerDashboardLayout({ children }: { children: ReactNode }) {
  const { isSharer, loading } = useAuth();
  ...
  }, [isSharer, loading, router]);

  if (loading) {
    return (
      <div className="flex items-center justify-center h-full p-8">
        <Icons.Logo className="h-10 w-10 animate-spin text-primary" />
        <p className="ml-4 text-muted-foreground">Loading Sharer Dashboard...</p>
      </div>
    );
  }
